...
#pragma warning restore 0649
        ...
        private void UpdateTooltips()
        {
            ...
            var allowedGameObjects = new List<GameObject>();
            var parentView = (BaseView)null;
            foreach (var ui in objects)
            {
                var myView = ui.GetComponentsInParent<BaseView>().FirstOrDefault(v => v.VisibleState != VisibleState.Closed);

                if (parentView == null)
                    parentView = myView;

                if (parentView == null || myView == parentView)
                    allowedGameObjects.Add(ui);
            }
...
#if UNITY_STANDALONE || UNITY_WEBGL || UNITY_EDITOR
                if (moved && !tooltip.ShowWhenMouseMoving)
                    break;
#endif
                //start show tooltip
        ...
        private void StartShowTooltip(Tooltip tooltip, bool forced)
        {
                ...
                else
                {
                    ...
                    }else
                    {
                        if (tooltip.TooltipInstance.VisibleState == VisibleState.Hidden && forced)
                        {
                            ...
                            if (owner.VisibleState == VisibleState.Visible)
                            {
                                timeToClose = tooltip.MaxDuration;
            ...
            timeToShow = forced ? 0 : tooltip.TooltipDelay;
        }

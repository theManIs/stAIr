...
namespace CometUI
{
    public class TooltipView : BaseView
    {
        ...
        protected override void OnBuild(bool isFirstBuild)
        {
            ...
            if (tooltip.Appearing == PlaceAppear.NearMouse)
            {
                var pos = Input.mousePosition;
                var size = GetSizeOfRectTransform(RectTransform);
                pos += new Vector3(size.x + tooltip.MouseOffset.x * 2, -size.y - tooltip.MouseOffset.y * 2, 0) / 2;
                Place(RectTransform, pos, tooltip.KeepInScreen);
            }
            else
            {
                PlaceAround(RectTransform, rect, tooltip.Appearing, tooltip.KeepInScreen, false, ref flipped);
            }

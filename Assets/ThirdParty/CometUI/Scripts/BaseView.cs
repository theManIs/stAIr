...
namespace CometUI
{
    public abstract class BaseView : MonoBehaviour, IBaseViewInternal
    {
        ...
        public void DestroyDynamicallyCreatedChildren()
        {
            GetComponentsInChildren<BaseView>().Where(f => f != this && f.IsDynamicallyCreated).ToList().ForEach(f => DestroyImmediate(f.gameObject));
        }
        ...
        #region ShowDialogCoroutine, ShowDialogAsync

        /// <summary>Shows the view and wait while it will be closed</summary>
        public IEnumerator ShowDialogCoroutine(BaseView owner, Action onAnimationDone = null, bool noAnimation = false, bool closeChildrenIfOpened = false)
        {
        ...
        /// <summary>Shows the view and wait while it will be closed</summary>
        public async Task ShowDialogAsync(BaseView owner, Action onAnimationDone = null, bool noAnimation = false, bool closeChildrenIfOpened = false)
        {
            Show(owner, onAnimationDone, noAnimation, closeChildrenIfOpened);
